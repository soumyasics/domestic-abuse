...
//import { viewCounsellorReqsForAdmin, approveCounsellorById, rejectCounsellorById } from '../../../Services/apiService';
...
const AdminCounsellorRequests = () => {
  ...
  const fetchCounsellors = useCallback(async (page = 0) => {
    try {
    //   const response = await viewCounsellorReqsForAdmin(page);
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
            //   const response = await approveCounsellorById(id);
            //   if (response.success) {
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
            //   const response = await rejectCounsellorById(id);
            //   if (response.success) {
          ...
          <Table striped bordered hover className="counsellors-table">
            <thead>
              <tr className="text-center">
                ...
                <th className='bg-purple text-white'>Availability</th>
            ...
            <tbody className='text-center'>
              {counsellors.map((counsellor, index) => (
                <tr key={counsellor._id}>
                  ...
                  <td>{counsellor.availability}</td>
                  <td>{counsellor.location}</td>
                  <td className=''>
                    <div className='text-center'>
                      <i className="m-3" onClick={() => {/* navigate to detailed view */}}><BsEye size={22} /></i>
                      <Button
