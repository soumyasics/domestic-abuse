...
import './LegalProfessionalViewBlogs.css';
import { deleteBlogsById, getBlogsByLPId, IMG_BASE_URL } from '../../../Services/apiService'; // Assuming getBlogs is the function to fetch blogs from backend
...
function LegalProfessionalViewBlogs() {
  ...
  const fetchBlogs = async () => {
    try {
      const response = await getBlogsByLPId(localStorage.getItem('lpId'));
        ...
        <div className='col'>
          ...
          <div className='row m-5'>
            <div className='col text-end'>
              <span><Link to={`/legal-professional-edit-blogs/${blog._id}`}><button className='btn rounded bg-purple px-5 m-2 text-white'>Edit</button></Link></span>
          ...
          <ToastContainer/>
            <div className='row m-5 mb-2'>
                <div className='col text-center theme-purple'>
                    <h5 className=''>Legal Professional Blogs</h5>
                    <h3 className='legal-professional-view-blogs-heading-border border-bottom border-5 rounded w-50 d-flex justify-content-center m-auto'>
...
export default LegalProfessionalViewBlogs;
